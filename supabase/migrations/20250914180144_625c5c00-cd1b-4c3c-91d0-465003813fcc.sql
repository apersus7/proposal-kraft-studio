-- Fix infinite recursion by dropping conflicting policies
...
-- Recreate the token-based policy to include email-based shares without conflicts
DROP POLICY IF EXISTS "Token-based proposal access" ON public.proposals;

-- Create a comprehensive policy that handles all access types without recursion
CREATE POLICY "Comprehensive proposal access" 
ON public.proposals 
FOR SELECT 
USING (
...
-- Fix the analytics policy to replace the existing one
...
CREATE POLICY "Comprehensive analytics insert" 
